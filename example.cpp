...
//   // run as: [program name] "0 -c" abc -a 1 -sdfl --flag -v 1 2.7 3 4 9 8.12
//   87
...
//   ")); double sum = 0; for (auto& d : v) sum += d; std::cout << "sum: " <<
//   sum << std::endl; auto f = parser.getv<std::string>("files"); std::cout <<
//   "files: "; std::copy(f.begin(), f.end(),
